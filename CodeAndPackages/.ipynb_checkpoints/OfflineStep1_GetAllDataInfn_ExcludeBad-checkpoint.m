...
sponids = extractIds(sponRecording);

%Motion correction data from spontanous behavior recording files
[sponPixShift, Files] = PixShiftLoad(WorkFolder);

DataFolder=[ProcessFolder 'Data\'];

%Copy spontanous behavior recording files to the DataFolder
copyInitialRecordedFolders(sponRecording, DataFolder);



idRanges=[1;99];

%Tif folders in total
[TifGenInfo,TifList, TifIDs] = getExpInfoFiles_NonMat(DataFolder, idRanges);
TBLtif=table(TifIDs(:),'VariableNames',{'FileID'});


%Recording file with Experiment informaiton stored in MatFiles
[TBLmat, TBLInfo] = ExtractExp_FromMat(DataFolder);
TBLmat.MatInfo=repmat(1,size(TBLmat,1),1);

%Merge the Tiff folder and Mat Table
TBLtifMat = outerjoin(TBLtif, TBLmat, 'Keys', 'FileID', 'MergeKeys', true);
temp1=ismember(TBLtifMat.FileID,sponids);
TBLtifMat.MatInfo(temp1)=0;
TBLtifMat.motionMed(temp1)=sponPixShift(:);


%Data with too much Pixshift were excluded for further analysis. 
MotionTh=10;
TBLtifMat=TBLtifMat(TBLtifMat.motionMed<MotionTh,:);
[~,DelIdx]=setdiff(TifIDs,TBLtifMat.FileID);

%%Move unqulified file folders to ExcludeFolder
RemovedList=TifList(DelIdx);
RemoveFolder=[DataFolder 'ExcludeFolder\'];
mkdir(RemoveFolder);
copyInitialRecordedFolders(RemovedList, RemoveFolder, DataFolder)
DelFolders(RemovedList, DataFolder) 
%%Move unqulified file folders to ExcludeFolder
...
removeMP=[];
for iFile=1:length(RemoveList)
    removeMP(iFile,1)=length(RemoveList{iFile})/RemoveFrame/length(confSet.ETL);
end

MPnumPerFile=10;
TiffID=extractIds(TiffTable.name);
FromRemoveTifTable=table(TiffID(:),removeMP(:),'VariableNames',{'FileID','MPnum'});
FromRemoveTifTable.Name=TiffTable.name;

tempTable=join(TBLtifMat,FromRemoveTifTable,"Keys","FileID");

RemoveI=find(tempTable.MatInfo==1&tempTable.MPnum<MPnumPerFile);
%%Move unqulified file folders to ExcludeFolder
RemovedList=tempTable.Name(RemoveI);
copyInitialRecordedFolders(RemovedList, RemoveFolder, DataFolder)
DelFolders(RemovedList, DataFolder) 
tempTable(RemoveI,:)=[];
FinalTBLtifMat=tempTable;
...
a=TBLmat(ValidID,:);
TBLtifMat = outerjoin(TBLtif, a, 'Keys', 'FileID', 'MergeKeys', true);
...
caTrialsInd=[7 8 66]
caTrials2=XMLread_Basic(DataFolder,caTrialsInd);
