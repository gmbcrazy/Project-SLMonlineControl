...
for (subdir in subdirs) {
  ...
  l2 <- lmer(Response ~ (SpeedR+SensoryR) * (TargetSpeedR+TargetSensoryR) + SpeedR*Speed + DistGroup+(1|Session),data = subset(dataTable, Sensory == 0 & NonTargetCell == 1),REML = FALSE)
  ...
  l4 <- lmer(Response ~ (SpeedR+SensoryR) * (TargetSpeedR+TargetSensoryR) + SpeedR*Speed + DistGroup+(1|Session),data = subset(dataTable, Sensory == 1 & NonTargetCell == 1),REML = FALSE)
