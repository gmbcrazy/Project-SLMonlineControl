%% Hari note
%this code will do motion correction and will return the three output
%reference image 06132022_SL172_Div1_Plane01_avFrame
%average chanel imag 06132022_SL172_Div1_Plane01_Ch2_AV
%To run the experiment we to update the exel table (2021Opto&BotoxSummary C\test)
%arrange the exp folder with date and mouse ID like 06132022_SL172
%rowsofInterest is the no of row where we put exp infomration in exel
%This code will avg the data and gives the different planes
%i.e. I recorded 402 plane with 32 avg, It will returns 402 folders with 32
%avg, I need to put the no of plane infomration on exel file
%To run this code need to run "MasterScriptCa2021_Part1.m"
...
function [] = MotionCorrection2020MorePadding(tempCaDir, processedDir, excelfilename, rowsOfInterest)
...
%         vOut.(fieldname) = vOutTemp;
    ...
    numPlanes = 1;
    if size(infoAllTrials.(fieldname).PVScan.Sequence,2) > 1%1
    ...
    for selectedPlane = 1:numPlanes % str2num(param.PlaneN{k,1})
        ...
        usfac = 20; % upsampling factor (integer) for dftregistration
        ...
        for i = 1:numel(caTrialsInd)
            ...
            % --- Load trial ChX ---
         %   [framesTrial] = readTifsAna2019([pathGet '\'
         %   caTrialCurr.name], num_x, num_y, ['Ch' param.ChForReg{k,1}]);
            framesTrial = framesTrialReg(:,:,selectedPlane:str2num(param.PlaneN{k,1}):size(framesTrialReg,3));
            ...
            imwrite (uint16(avTemp), [processedDir '\' experiment '_Plane' sprintf('%02i',selectedPlane) '_Ch' param.ChForReg{k,1} '_AV.tif'], 'WriteMode', 'append')%%modified by Hari
