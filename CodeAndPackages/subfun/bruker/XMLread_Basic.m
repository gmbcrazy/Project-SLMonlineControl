...
LastFrameSeqFile=[find(diff(FrameTS.relativeTime)<0);LastFame];
FirstFrameSeqFile=[1;LastFrameSeqFile(1:end-1)+1];
FrameSeq=zeros(size(FrameTS,1),1);
for icyc=1:length(LastFrameSeqFile)
    FrameSeq(FirstFrameSeqFile(icyc):LastFrameSeqFile(icyc))=icyc;
end

FrameTS.Seq=FrameSeq;
...
caTrials(iic).LastFrameSeqFile=LastFrameSeqFile;
