...
% 
function [rSpeed,rStim]=FunFilterCell(CaData,iscell,fSpeed,fStim,fs)

% XTimesStdTh = 3;
% MinInterVal = 20;
maxLag = 10;
deltaFoF=F2deltaFoF(double(CaData.F),double(CaData.Fneu),double(fs));
NeuroData=AmpNormalizeRow(deltaFoF',[0 100])';
NeuroDataCell=NeuroData(:,iscell);
numPlanes=size(fSpeed,2);
numPoint=length(iscell);
...
% rCenterIStim=intersect(NeedI,rIStim(1:TopCellN));
% rCenterISpeed=intersect(NeedI,rISpeed(1:TopCellN));

% AllFunctionI=union(rCenterIStim,rCenterISpeed);
% ColFunctionI=intersect(rCenterIStim,rCenterISpeed);
% randperm(5,3)
% NonFunctionI=setdiff(NeedI,AllFunctionI);
% 
