function UnMatchI=MPxmlExcuteMatchCheck(FileGenerateInfo,XMLTable,AllTestPoints3D,confSet)

MPxmlFiles=dir([FileGenerateInfo.tifFolder '*MarkPoints*.xml']);
framesMarkStimuli=[];
EXcuteMP=[];
EXcuteMPPointI=[];
DesignPointI=XMLTable(:,2);
...
%             nextCycle = min(find(cycleNumbers > markCycle));
            ...
            if temp2<=4                         %%<4 pixels difference of xy coordinates compared to original MarkPoints
               EXcuteMPPointI(i,1)=I1;
            else
               EXcuteMPPointI(i,1)=0;
            end
...
UnMatchI=find(ismember(DesignPointI,EXcuteMPPointI)==0);
MatchI=ismember(EXcuteMPPointI,DesignPointI)==0;
