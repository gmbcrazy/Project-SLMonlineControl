function [indexVector, stimulusIDVector, prePostStimuliVector] = getPSTHFrames_Suite2pBin(TotalMPointInd, PreSLMCal, PostSLMCal)
    % GetPSTHFrames - Generates an index vector with PreSLMCal frames before and PostSLMCal frames after each stimulus.
    %
    % Inputs:
    %   TotalMPointInd - Vector containing the cumulative frames where each stimulus occurs.
    ...
    % Iterate through each stimulus in TotalMPointInd
    for i = 1:length(TotalMPointInd)
        % Get the current stimulus frame
        currentStimulus = TotalMPointInd(i);
