function [GgraphOut,Res,r,p,tHandels]=ResSLMFunNetwork_ScoreNodeOrder_MetaFOV(PSTHparam,ResponseAdj,ResponseNode,ParamNet,ScoreCell,CellFOV)
...
CircosBand(end).Values=CellFOV(rankI(1:end-1),1);
CircosBand(end).Clim=[min(CellFOV) max(CellFOV)];
CircosBand(end).theta=theta;
CircosBand(end).Colormap=ScoreMap;
CircosBand(end).arc_gap=0;
...
bar2.Ticks=union(ScoreLim,0);
   ...
   ResParam.Color=ParamNet.FOVMap;
   ResParam.Marker='o';
...
[Res,r,p]=LuPairRegressPlotGroup_ExcludeDots(ScoreCell,Trans(1:end-1)',CellFOV,ParamNet.GroupTargetCell,ResParam);
colormap(t4,ParamNet.FOVMap);
set(gca,'clim',[min(CellFOV) max(CellFOV)]);
bar3=colorbar(t4);
bar3.Location='northoutside';
bar3.Position=[t4.Position(1)+0.15,t4.Position(2)+t4.Position(4)+0.01, 0.1,0.02];
bar3.Label.String=ParamNet.FOVLabel;
bar3.Ticks=union(ScoreLim,0);
