...
for iCell = 1:size(iscell, 1)
    ...
    tempNeuro=AmpNormalize(NeuroData(:,iscell(iCell)),[0 100]);
    tempStim=AmpNormalize(fStim(:,iPlane),[0 100]);

    [c(:,iCell), lags] = xcorr(tempNeuro, tempStim, maxLag, 'coeff');
...
while Continueloop
    ...
    rCenterIStim=intersect(CenterCloseI,rIStim(1:TopCellNRaw));  %%just avoid TopCellNRaw sensory cells.
