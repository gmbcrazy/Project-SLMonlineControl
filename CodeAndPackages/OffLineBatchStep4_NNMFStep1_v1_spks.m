...
% % 1) Outer join: puts GroupTargetCellN and scores into one table
% T = innerjoin(ValidSG, ValidnnmfSG, ...
%               'Keys', {'Session','Group'});
T = outerjoin(ValidSG, nnmfSG, ...
              'Keys', {'Session','Group'}, ...
              'MergeKeys', true);
SessTemp=[unique(ValidnnmfSG.Session)]
T=array2table([SessTemp zeros(size(SessTemp))+3],"VariableNames",{'Session','Group'})
T = outerjoin(T, nnmfSG, ...
              'Keys', {'Session','Group'}, ...
              'MergeKeys', true);
MergedValidSG = innerjoin(T, ValidSG, ...
              'Keys', {'Session','Group'});
...
% InValidnnmfSG=nnmfSG(InvalidI,:);
% ValidnnmfSG=nnmfSG(ValidI,:);
TempValidSG_DualScore1=ValidSG_DualScore(ValidI,:)
TempValidSG_DualScore2=ValidSG_DualScore;
TempValidSG_DualScore2(InvalidI,:)=[];

SessionUni=unique(TempValidSG_DualScore2.Session)
illG3=[];
for iS=1:length(SessionUni)
    I1=find(TempValidSG_DualScore2.Session==SessionUni(iS));
    if length(I1)==1
       if TempValidSG_DualScore2.Group(I1)==3
          illG3=[illG3;I1];
       end
    end
end
TempValidSG_DualScore2(illG3,:)=[];
MergedValidSG = TempValidSG_DualScore2(:,{'Session','Group','GroupTargetCellN'});
...
xlabel('SpeedScore');
ylabel('SensoryScore');
set(gca,'xlim',[-0.4 0.8],'ylim',[-0.2 0.4])
