...
%     handles.FileNameText.String = completeFileName;
    ...
    if isempty(AddToFileID)
        ...
        %In case of error happend during recording, the error data is
        %written to the original bin file, make a copy of original bin file
        %for back up
        %Check exsiting Copy.Bin to avoid overwrite
        ExistingPngList=dir([baseDirectory filesep '*CopyTSeries*.bin']);
        CopyFileID=length(ExistingPngList)+1;
        CopyFile=[baseDirectory filesep 'CopyTSeries' num2str(CopyFileID) '.bin'];
        copyfile([filePath '.bin'],CopyFile,'f');
