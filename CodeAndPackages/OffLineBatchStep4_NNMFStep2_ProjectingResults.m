%%
clear all
nnmfFolder='\\nimhlabstore1.nimh.nih.gov\UFNC\FNC2\Zhang\Projects\Project-LocalProcessing\Step4\awakeRefSpon\16-Dec-2025\NonNegMatFac\deltaF\';

load([nnmfFolder 'nnmfResult.mat'])
load([nnmfFolder 'ValidnnmfSG.mat'])
...
% TBLall = renameSubstringInTable(TBLall, 'Whisker', 'Sensory');
% TBLall_subNontarget = renameSubstringInTable(TBLall_subNontarget, 'Whisker', 'Sensory');



% for iFOV=1:length(IndexFOVNeed)
%     temp1 = SLMGroupTableTrial(SLMGroupTableTrial.Session == FOVres(iFOV).FOVid, :);
%     TrialID = unique(temp1.TrialID);
%     temp1 = SLMGroupTableTrial(SLMGroupTableTrial.Session == FOVres(iFOV).FOVid & SLMGroupTableTrial.TrialID == TrialID(1), :);
%     ProjScore(iFOV) = NNMFScoresFromNeuroTrace(FOVres(iFOV), NeuroTrace{FOVres(iFOV).FOVid}{iData},mean(BehTrace(FOVres(iFOV).FOVid).Speed,2),temp1.NonTargetCell==1);
%     CorrSpeed(iFOV,:)=corr([ProjScore(iFOV).Score(:,1) ProjScore(iFOV).Score_speedreg(:,1) ProjScore(iFOV).Score_sub(:,1) ProjScore(iFOV).Score_sub_speedreg(:,1)],mean(BehTrace(FOVres(iFOV).FOVid).Speed,2),'type','Pearson','rows','pairwise');
% end



%%Zero power does not needs exclude group, only exclude the session
AllTblPower0=TBLall(TBLall.PowerZero==1,:);
NTTbl_subPower0=TBLall_subNontarget(TBLall_subNontarget.PowerZero==1,:);

AllTblPower0= innerjoin(AllTblPower0, unique(MergedValidSG(:,{'Session'})), ...
                     'Keys', {'Session'});
NTTbl_subPower0= innerjoin(NTTbl_subPower0, unique(MergedValidSG(:,{'Session'})), ...
                     'Keys', {'Session'});
writetable(NTTbl_subPower0,[SaveFunFOVDim NDataName{iData} 'Power0NonTargetSLMScoreTrialDynWin' num2str(iWin) '.csv']);
writetable(AllTblPower0,[SaveFunFOVDim NDataName{iData} 'Power0AllSLMScoreTrialDynWin' num2str(iWin) '.csv']);


%%>0 power needs consider both valid session and group
AllTbl=TBLall(TBLall.PowerZero==0,:);
NTTbl_sub=TBLall_subNontarget(TBLall_subNontarget.PowerZero==0,:);
AllTbl=innerjoin(AllTbl , unique(MergedValidSG(:,{'Session','Group'})), ...
                     'Keys', {'Session','Group'});
NTTbl_sub=innerjoin(NTTbl_sub , unique(MergedValidSG(:,{'Session','Group'})), ...
                     'Keys', {'Session','Group'});
writetable(NTTbl_sub,[SaveFunFOVDim NDataName{iData} 'NonTargetSLMScoreTrialDynWin' num2str(iWin) '.csv']);
writetable(AllTbl,[SaveFunFOVDim NDataName{iData} 'AllSLMScoreTrialDynWin' num2str(iWin) '.csv']);



DataForCompare={AllTbl NTTbl_sub};
DataForComparePower0={AllTblPower0 NTTbl_subPower0};
...
SpeedThAll=[-1000 -1000 -1 -1  -0.5 0 0;...
             1000     0  0 1    0.5 1 1000];
clear statsSpeed;
close all
% SpeedTh=-2000;
...
%%T test, error plot
VOI={'SpeedScore','SensoryScore','SpeedScore_SpeedReg'};
clear stats

for iVar=1:length(VOI)

    for iSpeed=1:size(SpeedThAll,2)
        % GroupPair.SignY=0.02;
        % GroupPair.LimY=[-0.0001 GroupPair.SignY*1.2];
        % 
        SaveSubP1=[SaveFunFOVDimData num2str(SpeedThAll(1,iSpeed)) 'To' num2str(SpeedThAll(2,iSpeed)) '\'];
        mkdir(SaveSubP1);
        figure;
        % showYLim=[-0.5;1]*GroupPair.LimY(end)
        for iWhisk=1:length(ProcessPar.VolOut)

            GroupPair.SignY=0.03;
            GroupPair.SignY=GroupPair.SignY*iWhisk; 
            GroupPair.LimY=[-0.0001 GroupPair.SignY*1.2];
            showYLim=[-0.5 1]*GroupPair.LimY(end);
            ...
            for jtype=1:length(DataForCompare)
                ...
                tempD0Power0=DataForComparePower0{jtype};
    
                tempD0= tempD0(tempD0.Speed>SpeedThAll(1,iSpeed)&tempD0.Speed<SpeedThAll(2,iSpeed),:);
                tempD0Power0= tempD0Power0(tempD0Power0.Speed>SpeedThAll(1,iSpeed)&tempD0Power0.Speed<SpeedThAll(2,iSpeed),:);
    
            
    
                tempD=groupsummary(tempD0,{'Session','Group','Sensory'},groupMethod);
                tempD= groupsummaryBack2OldNames(tempD0, tempD, groupMethod);
    
                tempDPower0=groupsummary(tempD0Power0,{'Session','Sensory'},groupMethod);
                tempDPower0=groupsummaryBack2OldNames(tempD0Power0, tempDPower0, groupMethod);
    
        
    
        
                tempDNonZ=tempD(tempD.Sensory==ProcessPar.VolOut(iWhisk),:);
                tempDZero=tempDPower0(tempDPower0.Sensory==ProcessPar.VolOut(iWhisk),:);
                tempDZero.Group=zeros(size(tempDZero,1),1)+4;
    
    
                NeededGroup={'Session','Group','Sensory','SpeedScore','SensoryScore','SpeedScore_SpeedReg','SensoryScore_SpeedReg'};
                tempDNonZ=tempDNonZ(:,NeededGroup);
    
                tempDZero=tempDZero(:,NeededGroup);
    
                % tempDMerge=innerjoin(tempDNonZ,tempDZero,"Keys",{'Session'})
                tempDMerge=[tempDNonZ;tempDZero];
    
                for iGroup=1:4
                    tempDGroup{iGroup}=table2array(tempDMerge(tempDMerge.Group==iGroup,VOI{iVar}));
                end
        
              
                subplotLU(2,length(DataForCompare),iWhisk,jtype,[0.12 0.02 0.02 0.12 0.04 0.04]);
                stats(iVar,iWhisk,jtype)=ErrorBarPlotLU(1:4,tempDGroup,[],[ProcessPar.GroupColor;0.7 0.7 0.7],2,0,[],GroupPair,1:4);
        ...
        papersizePX = [0 0 18 18];
        set(gcf, 'PaperUnits', 'centimeters');
        set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
        print(gcf, [SaveSubP1 VOI{iVar} '.svg'], '-dsvg', '-painters');
        print(gcf, [SaveSubP1 VOI{iVar} '.tif'], '-dtiffn', '-painters');
                close all
    ...
    for iSpeed=1:size(SpeedThAll,2)
        ...
        for iWhisk=1:length(ProcessPar.VolOut)
            ...
            for jtype=1:length(DataForCompare)
                ...
                statsBox(iVar,iWhisk,jtype)=ErrorBoxPlotLU(1:4,tempDGroup,[ProcessPar.GroupColor;0.7 0.7 0.7],[],GroupPair,1:4)
                ylim(showYLim);
                yticks(union(0,showYLim))
                if jtype==1
                yticklabels(union(0,showYLim));
        ...
        set(gcf, 'PaperUnits', 'centimeters');
        set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
        print(gcf, [SaveSubP1 VOI{iVar} 'Box.svg'], '-dsvg', '-painters');
        print(gcf, [SaveSubP1 VOI{iVar} 'Box.tif'], '-dtiffn', '-painters');
        close all
    end
end
    ...
    for iSpeed=1:size(SpeedThAll,2)
        ...
        for iWhisk=1:length(ProcessPar.VolOut)
            ...
            for jtype=1:length(DataForCompare)
                ...
                % stats(iVar,iWhisk,jtype)=ErrorBarPlotLU(1:4,tempDGroup,[],[ProcessPar.GroupColor;0.7 0.7 0.7],2,0,[],GroupPair,1:4);
                % statsSpeed(iWhisk,jtype)=ErrorViolinHalf(1:4,tempDGroup,[ProcessPar.GroupColor;0.7 0.7 0.7],0,[],GroupPair,1:4);
                stats(iVar,iWhisk,jtype) = ErrorBarPlotPairTBL([1 2 3 4], tempDMerge, VOI{iVar}, 'Group', [ProcessPar.GroupColor;0.7 0.7 0.7], 2, 'Session');
...
% Setup
xGroup = [2 1 3];  % Plot group 2, then 1, then 3
colors = [1 0 0; 0 1 0; 0 0 1];  % Red, Green, Blue

% Call function
stats = ErrorBarPlotPairTBL(xGroup, DataTBL, 'Speed', 'Group', colors, 2, 'Session');
...
for iSpeed=1:size(SpeedThAll,2)
    ...
    % showYLim=[-0.5;1]*GroupPair.LimY(end)
    for iWhisk=1:length(ProcessPar.VolOut)
        showYLim=[-0.5;1]*GroupPair.LimY(end)*iWhisk;
        for jtype=1:length(DataForCompare)
            clear tempDGroup
            ...
            tempD=groupsummary(tempD0,{'Session','Group','Sensory'},'mean');
            tempD= groupsummaryBack2OldNames(tempD0, tempD, 'mean');

            tempDPower0=groupsummary(tempD0Power0,{'Session','Sensory'},'mean');
            tempDPower0=groupsummaryBack2OldNames(tempD0Power0, tempDPower0, 'mean');
            ...
            for iGroup=1:3
                tempDGroup{iGroup}=table2array(tempDNonZ.SensoryScore(tempDNonZ.Group==iGroup));
            end
                tempDGroup{iGroup+1}=table2array(tempDZero.SensoryScore);
    
          
            subplotLU(2,length(DataForCompare),iWhisk,jtype,[0.12 0.02 0.02 0.12 0.04 0.04]);
            % statsSensory(iWhisk,jtype)=ErrorBarPlotLU(1:4,tempDGroup,[],[ProcessPar.GroupColor;0.7 0.7 0.7],2,0,[],GroupPair,1:4);
            % statsSpeed(iWhisk,jtype)=ErrorViolinHalf(1:4,tempDGroup,[ProcessPar.GroupColor;0.7 0.7 0.7],0,[],GroupPair,1:4);
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
    print(gcf, [SaveSubP1 'SpensoryScores_box.svg'], '-dsvg', '-painters');
    print(gcf, [SaveSubP1 'SpensoryScores_box.tif'], '-dtiffn', '-painters');
...
clear statsSensory;
    ...
    GroupPair.LimY=[-0.0001 GroupPair.SignY*1.5];
    GroupPair.GroupName={'SLM(L)','SLM(L)+S','SLM(S)','SLM(S)+S','SLM(N)','SLM(N)+S','SLM(0)','SLM(0)+S'};
    GroupPair.ViolinLR=[0 0 0 0 0 0 0 0];
    GroupPair.SamplePairedPlot=0;
    Pair1=[1 2 3 4;5 6 7 8];
    Pair2=[1 1 1 2 2 3;2 3 4 3 4 4];
    Pair3=Pair2+4;
    
    RGroup=[1 2 3 4 1 2 3 4];
    GroupPair.Pair=[Pair2 Pair3 Pair1];
...
figure;
    % showYLim=[-0.5;1]*GroupPair.LimY(end)
    % for iWhisk=1:length(ProcessPar.VolOut)
        showYLim=[-1;1]*GroupPair.LimY(end)*iWhisk;
        for jtype=1:length(DataForCompare)
            clear tempDGroup
            tempDGroup={};
            tempD0=DataForCompare{jtype};
            ...
            % % tempD0= tempD0(tempD0.Speed>SpeedThAll(1,iSpeed)&tempD0.Speed<SpeedThAll(2,iSpeed),:);
            % % tempD0= tempD0(abs(tempD0.Speed)<SpeedTh,:);
            % 
            % % tempD=tempD0;
            % 
            % tempD=groupsummary(tempD0,{'Session','Group','Sensory','PowerZero'},'mean');
            ...
            for iWhisk=1:length(ProcessPar.VolOut)
                tempDNonZ=tempD(tempD.Sensory==ProcessPar.VolOut(iWhisk),:);
                tempDZero=tempD(tempDPower0.Sensory==ProcessPar.VolOut(iWhisk),:);
                for iGroup=1:3
                    tempDGroup{end+1}=tempDNonZ.SensoryScore(tempDNonZ.Group==iGroup);
                end
                tempDGroup{end+1}=tempDZero.SensoryScore;
             end
            tempColor=repmat([ProcessPar.GroupColor;0.7 0.7 0.7],2,1);

            % tempDGroup = tempDGroup(reOrganizeI);
            % tempColor2 = tempColor(reOrganizeI,:);


            statsSensory(jtype)=ErrorBoxPlotLU([1:8],tempDGroup,tempColor,[],GroupPair,RGroup)
            statsSensory(jtype)=ErrorBarPlotLU(1:length(tempDGroup),tempDGroup,[],tempColor,2,0,[],GroupPair,RGroup);
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
    print(gcf, [SaveSubP1 'SensoryScores.svg'], '-dsvg', '-painters');
    print(gcf, [SaveSubP1 'SensoryScores.tif'], '-dtiffn', '-painters');
...
figure;
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
    print(gcf, [SaveSubP1 'SpeedScores_SpeedReg.svg'], '-dsvg', '-painters');
    print(gcf, [SaveSubP1 'SpeedScores_SpeedReg.tif'], '-dtiffn', '-painters');
...
for iWhisk=1:length(ProcessPar.VolOut)
    for jtype=1:length(DataForCompare)
        ...
        tempD0=DataForCompare{jtype};
        tempD0= tempD0(tempD0.Speed<SpeedTh,:);
        tempD=tempD0;
        ...
        % % 
        % for iGroup=1:3
        %     tempDGroup{iGroup}=tempDNonZ.WhiskerScore(tempDNonZ.Group==iGroup);
        % end
        %     tempDGroup{iGroup+1}=tempDZero.WhiskerScore;
        % 
        % 
...
for iSpeed=1:size(SpeedThAll,2)
    ...
    for iWhisk=1:length(ProcessPar.VolOut)
        ...
        for jtype=1:length(DataForCompare)
            ...
            % statsSpeed(iWhisk,jtype)=ErrorBarPlotLU(1:4,tempDGroup,[],[ProcessPar.GroupColor;0.7 0.7 0.7],2,0,[],GroupPair,1:4);
            % statsSpeed(iWhisk,jtype)=ErrorViolinHalf(1:4,tempDGroup,[ProcessPar.GroupColor;0.7 0.7 0.7],0,[],GroupPair,1:4);
            statsSpeed(iWhisk,jtype) = ErrorBarPlotPairTBL([1 2 3 4], tempDMerge, 'SpeedScore', 'Group', [ProcessPar.GroupColor;0.7 0.7 0.7], 2, 'Session');
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
    print(gcf, [SaveSubP1 'SpeedScoresPair.svg'], '-dsvg', '-painters');
    print(gcf, [SaveSubP1 'SpeedScoresPair.tif'], '-dtiffn', '-painters');
...
close all
    ...
    for iWhisk=1:length(ProcessPar.VolOut)
        ...
        for jtype=1:length(DataForCompare)
            ...
            statsSpeed(iWhisk,jtype) = ErrorBarPlotPairTBL([1 2 3 4], tempDMerge, 'SpeedScore_SpeedReg', 'Group', [ProcessPar.GroupColor;0.7 0.7 0.7], 2, 'Session');
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', papersizePX, 'PaperSize', papersizePX(3:4));
    print(gcf, [SaveSubP1 'SpeedScores_SpeedRegPair.svg'], '-dsvg', '-painters');
    print(gcf, [SaveSubP1 'SpeedScores_SpeedRegPair.tif'], '-dtiffn', '-painters');
...
for iWhisk=1:length(ProcessPar.VolOut)
    for jtype=1:length(DataForCompare)
        ...
        subplotLU(2,length(DataForCompare),iWhisk,jtype);
        statsSpeed(iWhisk,jtype)=ErrorBarPlotLU(1:3,tempDGroup,[],[ProcessPar.GroupColor;0.7 0.7 0.7],2,0,[],GroupPair);
