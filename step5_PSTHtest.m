...
PlaneZ=[0 50 100];
clear PSTHtemp;
BinFile=dir([DataFolder '*TSeries*Laser1.1Point1.bin'])

FrameTotal=27*nPlane
Skip=PauseInd*3+1;

FileCount=[0,0];
% FrameTotal=
% BinFile=dir([DataFolder 'TSeries-03152024-0916-044Laser1.2Point8.bin']);
for iFile=1:length(BinFile)
...
Ly=BinData(1);
Lx=BinData(2);
if floor(length(BinData)/512/512)>FrameTotal
BinDataAll=BinData([1:Ly*Lx*(Skip-1) Skip*Ly*Lx+1:(FrameTotal+1)*Ly*Lx]);
...
PSTHtemp(:,:,:,iFile)=nanmean(Y(:,:,PostInd,:),3)-nanmean(Y(:,:,PreInd,:),3);
end
FileCount
