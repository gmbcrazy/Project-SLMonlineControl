function [indexVector, stimulusIDVector, prePostStimuliVector] = getPSTHFrames_MPxmlInfo(markCycle, PreSLMCal, PostSLMCal,MPFrameJump)
    ...
    if istable(markCycle)
       markCycle=markCycle.markCycle;
    end
    ...
    % Iterate through each stimulus in markCycle
    for i = 1:length(markCycle)
        % Get the current stimulus frame
        currentStimulus = markCycle(i);
