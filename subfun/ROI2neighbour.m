...
for iC=1:length(cellIDNeed)
    ...
    MedCenter(iC,1)=max(min(round(median(ypix)),m),1)+PixFromMedCenter;  %+PixFromMedCenter: noted that the imaging is extented PixFromMedCenter pixels at both head and tails
    MedCenter(iC,2)=max(min(round(median(xpix)),n),1)+PixFromMedCenter;  %+PixFromMedCenter: noted that the imaging is extented PixFromMedCenter pixels at both head and tails
    ...
    ROIboundary{iC}=bwboundaries(temp); % Store the boundary coordinates of the ROI
