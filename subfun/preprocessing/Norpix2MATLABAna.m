function [headerInfo, imgOut] = Norpix2MATLABAna(fileName, nF, showImg)
% July 2021
% Ana added var to read specific number of frames

% Updated in May 2021 by Jonathan Krynitsky based on original Norpix2MATLAB
% Optimized for Soohyun Lee Lab Grayscale Video Conversion
%   - Added conversion for Monochrome
%
% ARGUMENTS:
%    INPUTS:
%    FILENAME: String name/path of SEQ file
...
%    IMGOUT: Width x Height x NumImages
%
% SYNTAX:
...
%
% Original Norepix2MATLAB Written 04/08/08 by Brett Shoelson, PhD
...
if nargout > 1
    ...
    if ~isempty(nF)
        numFramesToProcess = nF; % JAK this was only 2 for some reason???
    else
        numFramesToProcess = headerInfo.AllocatedFrames; % JAK this was only 2 for some reason???
    end
    
    % To allocate a MATLAB array for the processed frames, use this:
    imgOut = uint8(zeros(headerInfo.ImageHeight,headerInfo.ImageWidth,2));
    imgOut(headerInfo.ImageHeight,headerInfo.ImageWidth,numFramesToProcess) = 0;
