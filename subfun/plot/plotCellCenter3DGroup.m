...
function plotCellCenter3DGroup(cellCenterGroup, Radius, varargin)

    if nargin < 4
        colorCell = jet(size(cellCenterGroup, 1));
        LineWidth = 1;
    elseif nargin == 4
    ...
    if size(colorCell, 1) == 1
        colorCell = repmat(colorCell, length(cellCenterGroup), 1);
    end

    if length(Radius) == 1
        Radius = repmat(Radius, length(cellCenterGroup), 1);
    end

    hold on;
    for iGroup=1:length(cellCenterGroup)
        if ~isempty(cellCenterGroup{iGroup})
