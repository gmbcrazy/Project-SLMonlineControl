function MultiMatrixImg2D(Data,Xstep,ComID,ComColor,InterLayerMapping)
...
for k=1:length(Data)
    ...
    MultiAdjCom2DTemp(Data{k},ComID{k},Xstart,ComColor);
    hold on;
end
if nargin==4
   return
...
function MultiAdjCom2DTemp(data,CT,XStart,varargin)
...
for i=1:length(CTall)
    plot([tempS(i)+0.5+XStart tempO(i)+0.5+XStart tempO(i)+0.5+XStart tempS(i)+0.5+XStart tempS(i)+0.5+XStart],[tempS(i)+0.5 tempS(i)+0.5 tempO(i)+0.5 tempO(i)+0.5 tempS(i)+0.5],'color',ComColor(i,:),'linewidth',2);hold on
end
