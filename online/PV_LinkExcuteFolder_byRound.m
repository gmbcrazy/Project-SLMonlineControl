          ...
          if numWholeFramesGrabbed > 0
             framesCounter = framesCounter + numWholeFramesGrabbed;
                if BreakYet==0 && framesCounter>=BreakPointFrame
                    disp(['Check Redundant' num2str(frameNum) ' ' num2str(framesCounter)])
...
%                           disp(['Redundant Detected' num2str(frameNum) ' ' num2str(framesCounter)])
                      ...
                      elseif CheckRedundant==1 && frameNum<=BreakPointFrame
                          ...
                          CheckRedundant=1;
                           if i == numWholeFramesGrabbed
                              CheckRedundant=0;
                              disp(['No Redundnant detected, and stop checking breakpoint']) 
                           end
%                       elseif CheckRedundant==1 && i == numWholeFramesGrabbed
%                          disp(['No Redundnant detected, and stop checking breakpoint']) 
%                          CheckRedundant=0;
%                       else
%                          disp(['Nothing is done' num2str(frameNum)])                 
                      end
...
%         Visulize samples recorded in each loop, only use for debugging!Lu Zhang
        ...
        % exit loop if finished if maxFrame frames were collected
            if started && frameNum >= maxFrame
                 running = 0;
                 disp([num2str(frameNum) ' frame saved from ' num2str(framesCounter) ' frames detected'])
