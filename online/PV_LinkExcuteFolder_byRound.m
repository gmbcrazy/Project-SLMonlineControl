...
% callback function
function RestFileList=PV_LinkExcuteFolder_byRound(ProcessFolder,RandomDelayInterval,maxFrame,BreakPointFrame,Round,ExcuteIndex)
...
%             end
    ...
    % clean up
         fclose(fileID);
         disp(['xml file ' num2str(ixml) ' from xmlList.csv is completed']);
...
function [MarkPointList,roundIDs,pointIDs,laserPowers]=GetXMLFile(MarkPointList,XMLpattern,Round)
    ...
    if ~isfield(MarkPointList,'gplname')
        for ixml=1:length(MarkPointList)
            MarkPointGPL(ixml)=dir([MarkPointList(ixml).folder '\R' num2str(roundIDs(ixml)) '*.gpl']);
