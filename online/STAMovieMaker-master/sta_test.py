...
class Worker(QObject):
    ...
    def work(self):
        ...
        if not error:
            ...
            all_stim_times = all_stim_times[
                (all_stim_times - pre_sec > min(frame_times)) & (all_stim_times + post_sec < max(frame_times))]
            ...
            for z in range(numZPlanes):
                ...
                if not est_frame_rate == frame_rate:
                    self.status_signal.emit(
                        'Error. Estimated frame rate: ' + str(est_frame_rate) + ', Specified frame rate: ' + str(
                            frame_rate))
                    time.sleep(2)
                ...
                for i in range(num_diff_stims):
                    ...
                    msg = 'Plane ' + str(z + 1) + ' of ' + str(numZPlanes) + '. Stim ' + str(i + 1) + ' of ' + str(
                        num_diff_stims) + ' (' + str(num_trials) + ' trials)'
                    ...
                    save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(i + 1) + '_STA_Plane' + str(
                        z + 1) + '_raw.tif'
                    ...
                    if methods:
                        for method in methods:
                            ...
                            if self.p['useSingleTrials']:
                                ...
                                save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(
                                    i + 1) + '_STA_Plane' + str(z + 1) + '_' + method + '_All' + str(
                                    num_trials) + 'Trials' + '.tif'
                            ...
                            save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(
                                i + 1) + '_STA_Plane' + str(z + 1) + '_' + method + '.tif'
                            tifffile.imsave(save_name, norm_movie)

                            avg_img[method][i] = sta.make_avg_image(norm_movie, avg_image_frames)
                            save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(
                                i + 1) + '_STA_Plane' + str(z + 1) + '_' + method + '_AvgImage' + '.tif'
                            tifffile.imsave(save_name, avg_img[method][i])

                            if self.p['colourByTime']:
                                results = sta.colour_by_time(norm_movie, avg_image_frames, smooth=int(frame_rate / 2),
                                                             useCorrelationImage=self.p['useCorrelationImage'],
                                ...
                                save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(
                                    i + 1) + '_STA_Plane' + str(z + 1) + '_' + method + '_TimePeak' + '.tif'
                                tifffile.imsave(save_name, rgb)

                                save_name = save_dir + os.sep + file_name_no_ext + '_Stim' + str(
                                    i + 1) + '_STA_Plane' + str(z + 1) + '_' + method + '_Corr' + '.tif'
                ...
                if num_diff_stims > 1:
                    for method in methods:
                        if self.p['makeMaxImage']:
                            save_name = save_dir + os.sep + file_name_no_ext + '_' + str(
                                num_diff_stims) + 'Stims' + '_STA_Plane' + str(
                                z + 1) + '_' + method + '_MaxResponseImage' + '.tif'
...
class MainWindow(QMainWindow, GUI.Ui_MainWindow):
    ...
    def loadMoviePath(self):
        movie_path = str(QFileDialog.getOpenFileName(self, 'Load movie', '',
                                                     'All files (*.*);;Binary (*.bin);;MPTIFF (*.tif);;Raw (*.raw)')[0])
        if movie_path:
    ...
    def loadStimOrderPath(self):
        stim_order_path = str(
            QFileDialog.getOpenFileName(self, 'Load stim order file', '', 'MATLAB (*.mat);;All files (*.*)')[0])
        if stim_order_path:
            self.stimOrder_lineEdit.setText(stim_order_path)
    ...
    def savePreset(self):
        filepath = str(
            QFileDialog.getSaveFileName(self, 'Save as preset...', self.presets_dir, 'Config file (*.cfg)')[0])
        if filepath:
