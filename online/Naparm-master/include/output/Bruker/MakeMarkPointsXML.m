...
for p = 1:NumRows
    ...
    Group{p} = ['Group ' num2str(PointNums(p))];
end
...
%%modified by Hari
Indices=handles.points.Idx;
NumCellsPerGroupCsum=(cumsum(NumCellsPerGroup))+1;
for ii=1:NumRows
   % IndicesTempF=Indices(1:NumCellsPerGroupCsum(ii));
    %IndicesTemp=Indices(NumCellsPerGroupCsum(ii):1:NumCellsPerGroupCsum(ii+1)-1);
   IndicesTemp=Indices(NumCellsPerGroupCsum(ii)-NumCellsPerGroup(ii):NumCellsPerGroupCsum(ii)-1); 
   IndicesTempF=num2str(IndicesTemp(1));
   IndicesTempL=num2str(IndicesTemp(end));
   IndicesTempFLCat=strcat(IndicesTempF,'-',IndicesTempL);
Indices_total(ii)={IndicesTempFLCat};
end
